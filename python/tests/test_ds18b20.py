...
def test_parse_scratchpad():
    assert parse_scratchpad(scratchpad(0x0191), 0x28) == 25.0625
    assert parse_scratchpad(scratchpad(-160), 0x28) == -10.0
    assert parse_scratchpad(scratchpad(50), 0x10) == 25.0  # DS18S20: /2
...
def test_read_full_flow(bridge, fw):
    ...
    probe._resolution = 9  # shorten the conversion sleep in tests
    assert probe.rom == rom(0x28, 42).hex()
    fw.ow_read_data = scratchpad(0x0155)  # 21.3125 C
    assert probe.read() == 21.3125
    # convert-T was sent with parasite power hold
...
def test_discover_filters_families(bridge, fw):
    fw.ow_devices = [rom(0x28, 1), rom(0x01, 2)]  # second is not a thermometer
    assert DS18B20.discover(bridge, 4) == [rom(0x28, 1).hex()]
